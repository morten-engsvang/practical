...
double open4point(double f(double), double a, double b, double delta, double eps, double f2, double f3,int numrecs){
	...
	if(error < tolerance){
		...
		double Q1 = open4point(f,a,(a+b)/2,delta/sqrt(2),eps,f1,f2,numrecs+1);
		double Q2 = open4point(f,(a+b)/2,b,delta/sqrt(2),eps,f1,f2,numrecs+1);
		return Q1+Q2;
	}
}
...
double recursive_integrate(double f(double), double a, double b, double delta, double eps){
	...
	/* Starter rekursionen */
	return open4point(f,a,b,delta,eps,f2,f3,numrecs);
}
